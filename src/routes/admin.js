    ...
    try {
        const ateliers = await query(`
            SELECT 
                ...
                COALESCE(a.budget_estime, 0) * COUNT(DISTINCT pl.creneau_id) as budget_total_atelier,
...
router.put('/budget/:atelierId', async (req, res) => {
    try {
        const { atelierId } = req.params;
        const { budget_estime } = req.body;
        
        await query(
            'UPDATE ateliers SET budget_estime = ? WHERE id = ?',
            [parseFloat(budget_estime) || 0, atelierId]
        );
