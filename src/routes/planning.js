        ...
        while (ateliersPlacables && iterations < maxIterations) {
            ...
            for (const creneauDebut of creneauxOrdonnes) {
                for (const atelier of ateliers) {
                    ...
                    // Règle spéciale : ateliers de 4 périodes (2 créneaux) doivent être le matin (P1-2 et P3-4)
                    // Jamais sur P3-4 + P6-7
                    if (nombreCreneaux === 2 && creneauxValides) {
                        const periodes = creneauxNecessaires.map(c => c.periode);
                        // Interdit : P3-4 suivi de P6-7
                        if (periodes.includes('P3-4') && periodes.includes('P6-7')) {
