...
router.post('/allouer', async (req, res) => {
    try {
        ...
        // Créer un map des INdisponibilités par enseignant
        const indispoMap = {};
        ...
        for (const atelier of ateliers) {
            ...
            for (let i = 0; i <= creneaux.length - nombreCreneaux; i++) {
                ...
                for (let j = 0; j < nombreCreneaux; j++) {
                    ...
                    // Vérifier disponibilité enseignant (disponible par défaut, sauf si marqué indisponible)
                    if (indispoMap[atelier.enseignant_acronyme] && 
                        indispoMap[atelier.enseignant_acronyme].has(creneau.id)) {
